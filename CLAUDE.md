...
HTTP 409. Intended audience is an internal office/workshop scenario; it ships
with a seed script for sample data, a **browser UI** for end-to-end use, and a
pytest suite. It is **not** hardened for production (dev server, no auth, SQLite,
hard-coded secret key).

**Architecture:** an application factory (`create_app` in `app.py`) wires the
SQLite database, registers three resource blueprints (`routes/rooms.py`,
`routes/bookings.py`, `routes/employees.py`), exposes `/health`, and serves the
single-page front-end (`static/index.html`, `static/app.js`, `static/styles.css`)
at `/`. The shared SQLAlchemy instance lives in `extensions.py`; models are in
`models.py`; overlap-detection logic is centralized in `utils/conflict.py`. The
database file is `db/bookings.db` (git-ignored) and is seeded by
`db/seed_data.py`. Tests live in `tests/` and run against an in-memory database.

The front-end is plain HTML/CSS/vanilla JS (no build step) served by Flask from
the same origin as the API, so **no CORS configuration is needed**. It calls the
JSON endpoints directly (`/rooms`, `/employees`, `/bookings`, ...).

**Endpoints:**

| Method | Path | Purpose |
|---|---|---|
| GET | `/` | Serve the single-page booking UI |
...
# then open http://localhost:5000 in a browser for the UI

pytest tests/ -v                # run the test suite (in-memory DB, no seed needed)
```

Note: `python app.py` works, but the idiomatic factory launch is
`flask --app app run` (both are fine now that the circular import is fixed).
...
- Tests: pytest 8 / pytest-cov (tests/)
- Front-end: plain HTML + CSS + vanilla JS (no framework, no build step), in static/
...
- **One blueprint per resource, verb_noun handlers.** Routes are grouped into
  blueprints named `<resource>_bp` (`rooms_bp`, `bookings_bp`, `employees_bp`)
  in `routes/` and registered in `create_app`. Handler functions are named
  `verb_noun` (`get_rooms`, `create_booking`, `reschedule_booking`,
  `cancel_booking`, `get_employees`).
...
- **SQLAlchemy 2.0-style lookups.** Fetch by primary key with
  `db.session.get(Model, id)` (not the legacy `Model.query.get(id)`), and use
  timezone-aware datetimes (`datetime.now(timezone.utc)`, not `utcnow()`).
...
- **Front-end is a static SPA served by Flask; edits need no restart.** The UI
  lives entirely in `static/` (`index.html`, `styles.css`, `app.js`, vanilla
  JS). It builds room/employee lookup maps from `/rooms` and `/employees` on
  load, then renders the Book / Bookings / Rooms tabs. Because Flask serves it
  as static files, changing `static/*` only requires a browser refresh — no
  server restart. Times are combined client-side into ISO strings
  (`YYYY-MM-DDTHH:MM:00`); remember **12 AM = 00:00**, **12 PM = 12:00**.

- **Tests use an in-memory DB via a `create_app` override.** `create_app`
  accepts an optional `test_config` dict; `tests/conftest.py` passes
  `{"TESTING": True, "SQLALCHEMY_DATABASE_URI": "sqlite:///:memory:"}` and its
  `client` fixture seeds exactly one room and one employee (no bookings), then
  yields the test client from inside an app context so tests can call HTTP
  endpoints *and* helpers like `check_overlap` directly. Run `pytest tests/ -v`;
  tests never touch `db/bookings.db`.
